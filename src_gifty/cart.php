...
if (isset($_POST['product_id'], $_POST['quantity']) && is_numeric($_POST['product_id']) && is_numeric($_POST['quantity'])) {
   ...
   // Set the post variables so we easily identify them, also make sure they are integer
    ...
    header('location: ind.php?page=cart');
    exit;
}
...
if (isset($_POST['placeorder']) && isset($_SESSION['cart']) && !empty($_SESSION['cart'])) {
    header('Location: ind.php?page=placeorder');
    exit;
}
    ...
    <form action="ind.php?page=cart" method="post">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td class="img">
                        <a href="ind.php?page=product&id=<?=$product['id']?>">
                            <img src="<?=$product['image']?>" width="50" height="50" alt="<?=$product['Name']?>">
                        </a>
                    </td>
                    <td>
                        <a href="ind.php?page=product&id=<?=$product['Product_ID']?>"><?=$product['Name']?></a>
                        <br>
                        <a href="ind.php?page=cart&remove=<?=$product['Product_ID']?>" class="remove">Remove</a>
                    </td>
                    <td class="price"><?=$product['Price']?> TND</td>
                    <td class="quantity">
                    ...
                    <td class="price"><?=$product['Price'] * $products_in_cart[$product['Product_ID']]?> TND</td>
                </tr>
                <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
        ...
        <div class="subtotal">
            ...
            <span class="price"><?=$subtotal?> TND</span>
        </div>
        <div class="buttons">
